...
func encodeFixedBytes(v reflect.Value) ([]byte, error) {
	...
	if v.Kind() == reflect.String {
		decode, err := decodeHexStr(v.String())
		if err != nil {
			return nil, err
		}
		v = reflect.ValueOf(decode)
	}
...
func encodeSliceAndArray(v reflect.Value, t *Type) ([]byte, error) {
	if v.Kind() != reflect.Array && v.Kind() != reflect.Slice {
		return nil, encodeErr(v, t.String())
	}
	...
	if t.kind == KindArray && t.size != v.Len() {
		return nil, fmt.Errorf("array len imcompatible")
	}
	var ret, tail []byte
	...
	offset := 0
	isDynamic := t.elem.isVariableInput()
	if isDynamic {
...
func encodeTuple(v reflect.Value, t *Type) ([]byte, error) {
	...
	for i, elem := range t.tuple {
		...
		if aux.Kind() == reflect.Invalid {
			return nil, fmt.Errorf("connot get key %s", elem.Name)
		}
...
func encodeBool(v reflect.Value) ([]byte, error) {
	...
	if v.Bool() {
		return leftPad(big.NewInt(1).Bytes(), 32), nil
	}
	return leftPad(big.NewInt(0).Bytes(), 32), nil
}
...
func packBytesSlice(buf []byte, l int) ([]byte, error) {
	length, err := encodeNum(reflect.ValueOf(l))
	if err != nil {
		return nil, err
	}
	return append(length, rightPad(buf, (l+31)/32*32)...), nil
}
...
func encodeNum(v reflect.Value) ([]byte, error) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		return toU256(new(big.Int).SetInt64(v.Int())), nil
	...
	case reflect.String:
		if strings.HasPrefix(strings.ToUpper(v.String()), "0X") {
...
func decodeHexStr(v string) ([]byte, error) {
	s := v
	var decode []byte
	var err error
	if strings.HasPrefix(strings.ToUpper(s), "0X") {
		decode, err = hexutil.Decode(s)
	} else {
		decode, err = hexutil.Decode("0x" + s)
	}
	return decode, err
}

func encodeHexStr(b []byte) string {
